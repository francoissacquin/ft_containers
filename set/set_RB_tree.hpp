...
#include "./set_rb_tree_iterator.hpp"
#include <iostream>
#include <string>
...
public:
	...
	explicit set_Node(T const & d = T()): data(d)
	{
		//nothing here
	}

	bool	operator==( const set_Node & rhs )
	{
		return (this->data == rhs.data);
	}

	set_Node &	operator=( const set_Node & rhs )
	{
		if (this != &rhs)
		{
			data = rhs.data;
...
public:
	...
	typedef set_Node<value_type>							node;
	typedef Alloc											allocator_type;
	typedef set_rb_tree_iterator<value_type, node>			iterator;
	...
	//CONSTRUCTOR//
	set_RB_tree( const allocator_type & alloc = allocator_type()): _pair_alloc(alloc), _root(NULL), _size(0)
	{
	...
	set_RB_tree &		operator=( const set_RB_tree & rhs )
	{
		if (this != &rhs)
		{
			clear();
	...
	pair<iterator, bool>	insert(value_type d)
	{
		...
		{
			y = x;
			if (key_compare() (new_node->data, x->data)) // we go down the tree to place the new_node by comparing its data value with existing nodes.
			{
				x = x->left;
			}
			else if (key_compare() (x->data, new_node->data))
			{
				x = x->right;
			}
			else
		...
		else if (key_compare() (new_node->data, y->data)) // we check by comparing data values wether new_node goes to the left or right of its parent
		{
			y->left = new_node;
		}
		else
...
private:
	...
	NodePtr		search_tree_helper(NodePtr n, key_type k) const
	{
		...
		if (key_compare() (k, n->data)) // we progress throught he tree by comparing data values, in case the data is a pair, the pair operator overloads compare key first
		{
			return search_tree_helper(n->left, k);
		}
		else if (key_compare() (n->data, k))
		{
	...
	NodePtr		lower_bound_helper(NodePtr x, key_type k) const
	{
		...
		if (key_compare() (y->data, k) && x == _TNULL)
			return _TNULL;
		return y;
	}
	...
	void		delete_node_helper(NodePtr n, value_type k)
	{
		...
		{
			if ((!(key_compare() (n->data, k))) && (!(key_compare() (k, n->data))))
			{
				z = n;
				break ;
			}
...
template<typename T>
set_Node<T> *		successor(set_Node<T> * x)
{
	if (x->left == NULL)
		return x;
	// if the right sub-tree is not null, the successor is the leftmost node in the right sub-tree
...
//find the predecessor of a given node
template<typename T>
set_Node<T> *		predecessor(set_Node<T> * x)
{
	if (x->left == NULL)
	{
		set_Node<T> *	y = x->parent;
